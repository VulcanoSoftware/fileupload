...
def resolve_path(base_path, virtual_path):
    base_abs = os.path.abspath(base_path)
    virt = normalize_virtual_path(virtual_path)
    rel = os.path.normpath(virt.lstrip('/'))
    if rel in ('', '.'):
        candidate = base_abs
    else:
        candidate = os.path.abspath(os.path.join(base_abs, rel))
    try:
        common_root = os.path.commonpath([base_abs, candidate])
    except ValueError as exc:
        raise OSError(errno.EPERM, "Invalid path resolution") from exc
    if common_root != base_abs:
        raise OSError(errno.EPERM, "Path traversal outside base path is not allowed")
    return candidate
...
def start_webdav_server_thread(webdav_config, upload_src_path, webhook_url, serve_root_path=None):
    ...
    if not serve_root_path:
        raise RuntimeError("WebDAV requires a FUSE mount path as root.")
    dav_root = os.path.abspath(serve_root_path)
    if not os.path.isdir(dav_root):
        raise RuntimeError(f"WebDAV root path does not exist or is not a directory: {dav_root}")
...
class FileBalancerSFTPServer(asyncssh.SFTPServer):
    def __init__(self, chan, base_path):
        self._base_path = os.path.abspath(base_path)
        if not os.path.isdir(self._base_path):
            raise RuntimeError(f"SFTP root path does not exist or is not a directory: {self._base_path}")
        super().__init__(chan)
    ...
    def _resolve_real_path(self, path):
        virt = self._normalize_virtual(path)
        return virt, resolve_path(self._base_path, virt)

    @staticmethod
    def _attrs_from_stat(st):
        # Start from asyncssh's native mapping so all metadata flags are set correctly.
        attrs = SFTPAttrs.from_local(st)
        attrs.permissions = int(getattr(st, 'st_mode', 0))
        attrs.size = int(getattr(st, 'st_size', 0))
        attrs.atime = int(getattr(st, 'st_atime', time.time()))
        attrs.mtime = int(getattr(st, 'st_mtime', time.time()))
        if hasattr(st, 'st_uid'):
            attrs.uid = int(st.st_uid)
        if hasattr(st, 'st_gid'):
            attrs.gid = int(st.st_gid)
        return attrs

    @staticmethod
    def _raise_sftp_os_error(exc, virt):
        if isinstance(exc, FileNotFoundError):
            raise asyncssh.SFTPNoSuchFile(virt) from exc
    ...
    async def scandir(self, path):
        virt_dir, real_dir = self._resolve_real_path(path)
        try:
            ...
            for name in sorted(os.listdir(real_dir)):
                child_path = os.path.join(real_dir, name)
                st = os.lstat(child_path)
                attrs = self._attrs_from_stat(st)
                # longname must not contain an arbitrary path; clients use attrs to detect file-vs-dir
                yield SFTPName(name.encode('utf-8'), None, attrs)
        except OSError as exc:
            self._raise_sftp_os_error(exc, virt_dir)

    async def _stat_helper(self, path, follow_symlinks=True):
        virt, real_path = self._resolve_real_path(path)
        try:
            if follow_symlinks:
                st = os.stat(real_path)
            else:
                st = os.lstat(real_path)
            return self._attrs_from_stat(st)
        except OSError as exc:
            self._raise_sftp_os_error(exc, virt)
    ...
    async def rename(self, oldpath, newpath, flags=0):
        old_virt, old_real = self._resolve_real_path(oldpath)
        new_virt, new_real = self._resolve_real_path(newpath)
        try:
            os.rename(old_real, new_real)
        except OSError as exc:
            self._raise_sftp_os_error(exc, old_virt + " -> " + new_virt)


def start_s3_server_thread(s3_config, webhook_url, serve_root_path):
    if not s3_config.get('enabled', False):
        return
    if not serve_root_path:
        raise RuntimeError("S3 requires a FUSE mount path as root.")

    host = s3_config.get('host', '0.0.0.0')
    port = int(s3_config.get('port', 9000))
    bucket_name = s3_config.get('bucket_name', 'storage')
    base_path = os.path.abspath(serve_root_path)
    if not os.path.isdir(base_path):
        raise RuntimeError(f"S3 root path does not exist or is not a directory: {base_path}")

    class S3PassthroughHandler(BaseHTTPRequestHandler):
        server_version = "FileBalancerS3/1.0"
        root_path = base_path
        only_bucket = bucket_name

        def _send_bytes(self, status_code, payload=b"", content_type="application/octet-stream"):
        ...
        def _send_xml(self, status_code, xml_body):
            self._send_bytes(status_code, xml_body.encode('utf-8'), "application/xml")

        def _split_request_path(self):
            parsed = urlparse(self.path)
            parts = [p for p in parsed.path.split('/') if p]
            query = parse_qs(parsed.query)
            return parts, query

        def _resolve_object_path(self, key):
            if not key:
                raise OSError(errno.EINVAL, "Object key is required")
            return resolve_path(self.root_path, "/" + key)

        def _list_bucket_xml(self):
            names = sorted(os.listdir(self.root_path))
            object_entries = []
            for name in names:
                child = os.path.join(self.root_path, name)
                if os.path.isfile(child):
                    object_entries.append(
                        "<Contents><Key>{}</Key><Size>{}</Size></Contents>".format(
                            xml_escape(name),
            ...
            return (
                ...
                '<ListBucketResult>'
                f'<Name>{xml_escape(self.only_bucket)}</Name>'
                + ''.join(object_entries)
                + '</ListBucketResult>'
            )
        ...
        def do_GET(self):
            try:
                parts, query = self._split_request_path()
                if not parts:
                    now = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.000Z")
                    xml_body = (
                        '<?xml version="1.0" encoding="UTF-8"?>'
                        '<ListAllMyBucketsResult><Buckets>'
                        f'<Bucket><Name>{xml_escape(self.only_bucket)}</Name><CreationDate>{now}</CreationDate></Bucket>'
                        '</Buckets></ListAllMyBucketsResult>'
                    )
                    self._send_xml(200, xml_body)
                    return

                bucket = unquote(parts[0])
                if bucket != self.only_bucket:
                    self._send_bytes(404, b"Bucket not found", "text/plain")
                    return

                if len(parts) == 1:
                    if "list-type" in query or "prefix" in query or not query:
                        self._send_xml(200, self._list_bucket_xml())
                        return
                    self._send_bytes(400, b"Unsupported query", "text/plain")
                    return

                key = unquote('/'.join(parts[1:]))
                real_path = self._resolve_object_path(key)
                ...
                if isinstance(exc, FileNotFoundError):
                    ...
                    self._send_bytes(500, str(exc).encode('utf-8', errors='ignore'), "text/plain")

        def do_PUT(self):
            try:
                parts, _query = self._split_request_path()
                if len(parts) < 2:
                    self._send_bytes(400, b"Path must include /bucket/key", "text/plain")
                    return
                bucket = unquote(parts[0])
                if bucket != self.only_bucket:
                    self._send_bytes(404, b"Bucket not found", "text/plain")
        ...
        def log_message(self, format, *args):
            logger.debug("S3 %s - %s", self.address_string(), format % args)

    def run_server():
        try:
            httpd = ThreadingHTTPServer((host, port), S3PassthroughHandler)
            print_and_discord(f"S3 server started on {host}:{port} (bucket: {bucket_name}, root: {base_path})", webhook_url)
            httpd.serve_forever()
        except Exception as e:
...
def start_sftp_server_thread(sftp_config, upload_src_path, webhook_url, serve_root_path=None):
    if not sftp_config.get('enabled', False):
        return
    if not serve_root_path:
        raise RuntimeError("SFTP requires a FUSE mount path as root.")
    if not os.path.isdir(serve_root_path):
        raise RuntimeError(f"SFTP root path does not exist or is not a directory: {serve_root_path}")
...
def save_config_if_missing(config_data, config_path=config_path):
    ...
    s3_server_config = config_data.get('s3_server')
    if not isinstance(s3_server_config, dict):
        s3_server_config = {}
    ...
    s3_bucket_name = s3_server_config.get('bucket_name', 'storage')
    s3_upload_src = s3_server_config.get('upload_src', src)
    s3_use_fuse = s3_server_config.get('use_fuse_mount_as_root', True)
    ...
    lines.append(f"  enabled: {_bool_to_yaml(s3_enabled)}")
    lines.append(f"  host: {s3_host}")
    lines.append(f"  port: {s3_port}")
    lines.append(f"  bucket_name: '{s3_bucket_name}'")
    lines.append(f"  upload_src: {s3_upload_src}")
    lines.append(f"  use_fuse_mount_as_root: {_bool_to_yaml(s3_use_fuse)}")
...
def main():
    ...
    if not os.path.exists(config_path):
        ...
        shared_upload_src_input = input(f"Shared upload folder for WebDAV/SFTP/FUSE/S3 (default {src}): ").strip()
        ...
        s3_server_cfg = {}
        print("\nS3 server:")
        print("- Built-in S3-compatible API backed directly by the mounted filesystem.")
        print("- Bucket/object requests are passthrough OS filesystem operations.")
        use_s3 = input("Do you want to enable the S3 server? (yes/no): ").strip().lower() == 'yes'
        if use_s3:
            default_s3_host = s3_server_config.get('host', '0.0.0.0')
            default_s3_port = s3_server_config.get('port', 9000)
            default_s3_bucket = s3_server_config.get('bucket_name', 'storage')
            host_input = input(f"S3 host (default {default_s3_host}): ").strip()
            port_input = input(f"S3 port (default {default_s3_port}): ").strip()
            bucket_input = input(f"S3 bucket name (default {default_s3_bucket}): ").strip()
            use_fuse_input_s3 = input("Use FUSE mount as S3 root? (yes/no, default yes): ").strip().lower() != 'no'
            if port_input.isdigit():
                s3_port_value = int(port_input)
            else:
                s3_port_value = default_s3_port
            s3_server_cfg = {
                'enabled': True,
                'host': host_input or default_s3_host,
                'port': s3_port_value,
                'bucket_name': bucket_input or default_s3_bucket,
                'upload_src': shared_upload_src,
                'use_fuse_mount_as_root': use_fuse_input_s3,
            }
        else:
        ...
        s3_server_config = new_config.get('s3_server', {})
    ...
    if (webdav_enabled or sftp_enabled or s3_enabled) and not fuse_enabled:
        raise RuntimeError("WebDAV/SFTP/S3 require the FUSE mount to be enabled.")
    ...
    for upload_path in (webdav_upload_src, sftp_upload_src, s3_upload_src, fuse_upload_src):
        if upload_path:
    ...
    protocol_root_path = fuse_server_config.get('mount_point')
    start_webdav_server_thread(webdav_server_config, webdav_upload_src, webhook_url, serve_root_path=protocol_root_path)
    start_sftp_server_thread(
        sftp_server_config,
        sftp_upload_src,
        webhook_url,
        serve_root_path=protocol_root_path,
    )
    start_s3_server_thread(
        s3_server_config,
        webhook_url,
        serve_root_path=protocol_root_path,
    )
